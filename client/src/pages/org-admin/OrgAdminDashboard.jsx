  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6 page-enter">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-glow" style={{ color: '#f0e8fc' }}>
          Welcome back, {user?.name?.split(' ')[0] || 'Admin'}
        </h1>
        <p className="mt-1 text-sm" style={{ color: '#7060a0' }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <motion.div key={label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <p className="text-2xl font-bold" style={{ color: '#f0e8fc' }}>{value}</p>
          </motion.div>
        ))}
      </div>
      ...
      <div>
        ...
        {isLoading ? (
          ...
          <div className="glass-card p-12 text-center">
            ...
            <p style={{ color: '#7060a0' }}>No active cohorts at the moment.</p>
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {active.map((cohort, i) => (
              <motion.div key={cohort.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div>
                    ...
                    <p className="text-xs mt-0.5" style={{ color: '#7060a0' }}>{PROG_LABELS[cohort.program_type] || cohort.program_type}</p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div className="rounded-lg p-2" style={{ background: 'rgba(170,120,166,0.06)' }}>
                    <p className="text-base font-bold" style={{ color: '#f0e8fc' }}>{cohort.enrollment_count}</p>
                    <p className="text-xs" style={{ color: '#7060a0' }}>Participants</p>
                  </div>
                  <div className="rounded-lg p-2" style={{ background: 'rgba(170,120,166,0.06)' }}>
                    <p className="text-xs font-medium" style={{ color: '#f0e8fc' }}>
                      {cohort.start_date ? format(new Date(cohort.start_date), 'MMM d') : '—'}
                    </p>
                    <p className="text-xs" style={{ color: '#7060a0' }}>Start</p>
                  </div>
                  <div className="rounded-lg p-2" style={{ background: 'rgba(170,120,166,0.06)' }}>
                    <p className="text-xs font-medium" style={{ color: '#f0e8fc' }}>
                      {cohort.end_date ? format(new Date(cohort.end_date), 'MMM d') : '—'}
                    </p>
                    <p className="text-xs" style={{ color: '#7060a0' }}>End</p>
                  </div>
                </div>
